...
public class UserServiceImpl implements UserService {
    ...
    public void addUser(User user, String privateKey) throws Exception {
        ...
        // 给新用户2个代币
        WalletUtils.transferTo(web3j,adminCredentials,credentials.getAddress(),50.0);
        // 新用户调用合约进行注册 会消耗代币
    ...
    public User getUserInfo(String privateKey) throws Exception {
        ...
        log.info("priv----------" + credentials.getEcKeyPair().getPrivateKey());
        log.info("publ----------" + credentials.getEcKeyPair().getPublicKey());
        // 2.加载合约
        PictureCopyright contract = PictureCopyright.load(contractAddress, web3j, credentials, new DefaultGasProvider());
        log.info("----------" + contract);
        // 3.查询用户
        log.info("----------" + contract.getUserCopyrightsNumber(contractAddress).send());
